...
public class SystemState {
	...
	HashMap<String,String> pathid;				//Nuno: added this for Symbiosis; map: thread id -> path id
	int numUpdates;								//Nuno: added this for Symbiosis; counts the number of updates to path id
	...
	public SystemState (Config config, JVM vm) {
		...
		pathid = new HashMap<String, String>(); //Nuno: added this for symbiosis
		numUpdates = 0;
	...
	//Nuno: added this for Symbiosis {
	public String getPathId(String tid){
		if(!pathid.containsKey(tid))
		{
			pathid.put(tid, "-1");
		}
		return pathid.get(tid);
	}

	public void updatePathId(String tid, int decision){
		if(!pathid.containsKey(tid))
		{
			String path = "-1"+decision;
			pathid.put(tid, path);
		}
		else
		{
			String path = pathid.get(tid);
			path += decision;
			pathid.put(tid, path);
		}
		numUpdates++;
	...
	public void backtrackPathId(String tid)
	{
		if(pathid.containsKey(tid) && numUpdates > 1)
		{
			String path = pathid.get(tid);
			if(!path.equals("-1")){
