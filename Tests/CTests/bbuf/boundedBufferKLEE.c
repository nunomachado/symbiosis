...
#define MAX_SIZE 3 //substitutes bbuf->max_size (klee crashes with this var symbolic marked as symbolic due to % operation)
...
int bounded_buf_destroy(bounded_buf_t * bbuf)
{
    ...
    if (bbuf->rear >= bbuf->head ) {
        ...
        for (i = bbuf->head; i < MAX_SIZE; i++) free(bbuf->buf[i]);
...
int bounded_buf_put(bounded_buf_t * bbuf, void *item)
{
    ...
    if ( (bbuf->rear + 1)% MAX_SIZE == bbuf->head )
    {
        bbuf->p_wait++;
        
        while ( (bbuf->rear + 1)% MAX_SIZE == bbuf->head ){
    ...
    if (status == 0){
        ...
        bbuf->rear = (bbuf->rear+1)% (MAX_SIZE);
        if (bbuf->c_wait > 0){
...
int bounded_buf_get(bounded_buf_t *bbuf, void **item)
{
    ...
    if (status == 0)
    {
        assert(bbuf->head != bbuf->rear);
        *item = bbuf->buf[bbuf->head];
        bbuf->head = (bbuf->head+1) % MAX_SIZE;//bbuf->max_size;
...
int bounded_buf_is_full(bounded_buf_t* bbuf)
{
    ...
    retval = ( (bbuf->rear + 1) % MAX_SIZE == bbuf->head )? 1 : 0;
    status = pthread_mutex_unlock(&bbuf->mutex);
    return (status == 0)? retval : -1;
}
...
int main(int argc, char ** argv)
{
    ...
    klee_make_symbolic(&buffer, sizeof(buffer), "bbuf");
