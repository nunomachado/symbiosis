        ...
        int icase)
{
    ...
    preBmBc((char *) bmp->saved_x, m, bmp->bmBc);
...
bm_search(BM *bmp,
          char *y,
          size_t n,
          int (*mfun)(char *buf, size_t n, size_t pos, void *misc),
          void *misc)
{
    ssize_t i, j;
    int  c;
    ...
    while (j <= n - bmp->saved_m)
    {
        ...
        while(i >= 0){
            ...
            char tmpbmp = bmp->saved_x[i];
             klee_make_symbolic(&tmpbmp, sizeof(tmpbmp), "bmp->saved_x[i]");
            if(tmpbmp != tmpc)
                break;
            --i;
        }
        ...
        {
            char c = (bmp->icase ? tolower(y[i + j]) : y[i + j]);
