...
scan_file(char *pathname)
{
    ...
    fd = open(pathname, O_RDONLY); klee_make_symbolic(&fd, sizeof(fd),"fd");
    if (fd < 0)
    {
        if (verbose)
        {
    ...
    buf = (char *) mmap(NULL, len, PROT_READ, MAP_PRIVATE|MAP_NORESERVE, fd, 0);
             ...
             int f)
{
    ...
    //n_bytes += sp->st_size; //Nuno
    
    switch (f)
    {
        case FTW_F:
            pqueue_put(&pqb, (void *) mystrdup(path));
            return 0;
