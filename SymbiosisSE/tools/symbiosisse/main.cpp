...
//Note: Options for interpreter style option processing (command line execution) [http://llvm.org/docs/CommandLine.html#the-cl-consumeafter-modifier]
namespace {
  cl::opt<std::string>
...
public:
  ...
  std::ostream &getTraceStream() const { return *m_traceFile; }
  ...
  std::ostream *openTraceFile(const std::string &filename);
...
KleeHandler::KleeHandler(int argc, char **argv) 
  ...
  m_traceFile = openOutputFile("trace");    //:CLAP:
}
...
void KleeHandler::setTraceFile(const std::string &filename) {
	  m_traceFile = openOutputFile(filename);
}
...
//:CLAP:
std::ostream *KleeHandler::openTraceFile(const std::string &filename) {
  std::ios::openmode io_mode = std::ios::out | std::ios::binary | std::ios::app | std::ios::ate;
  std::ostream *f;
