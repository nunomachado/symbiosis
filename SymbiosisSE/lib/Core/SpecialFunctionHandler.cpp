...
#define addDNR(name, handler) { name, \
  ...
  add("myPThreadCreate", handleMyPThreadCreate,false),        //C calls

  add("_Z17myBeforeMutexLockP23_opaque_pthread_mutex_t",handleBeforeMutexLock,false),
  add("_Z16myAfterMutexLockP23_opaque_pthread_mutex_t",handleAfterMutexLock,false),

  //PThread related handlers
  add("pthread_create", handlePThreadCreate, true), //** Nuno: set to true
  ...
  add("pthread_mutex_lock", handlePThreadLock, false),
  add("pthread_mutex_unlock", handlePThreadUnlock, false),
  ...
  add("pthread_mutex_init", handlePThreadMutexInit, false),
  add("pthread_mutex_destroy", handlePThreadMutexDestroy, false),

  add("pthread_cond_init", handlePThreadCondInit, false),
  add("pthread_cond_destroy", handlePThreadCondDestroy, false),

  add("pthread_attr_init", handlePThreadAttrInit, false),
  add("pthread_attr_destroy", handlePThreadAttrDestroy, false),

  add("pthread_attr_setscope", handlePThreadAttrSetScope, false),
  //:CLAP:}
  ...
  for (Executor::ExactResolutionList::iterator it = rl.begin(), 
    ...
    if (res) {
      ...
      executor.executeMakeSymbolic(*s, mo, name,basevalue);
        ...
        std::cerr << "Name: "<<name<< " Size: "<<mo->getSizeExpr()<<"\n";//oh, it gets the memory object address
...
void SpecialFunctionHandler::handlePThreadJoin(ExecutionState &state,
          ...
          std::vector<ref<Expr> > &arguments)
  {
      ...
      //debug
      /*std::stringstream obj; //get the obj
      Instruction *inst = target->inst;
      obj << inst->getOperand(0);
      //cerr << "join parent thread "<< obj << "\n";
      
      int i = 0;
      for(User::op_iterator opIt = inst->op_begin(); opIt != inst->op_end(); ++opIt)
      {
          cerr << "("<<i<<") join parent thread " << opIt->get() << "\n";
          i++;
      }
      
      i = 0;
      for(std::vector<ref<Expr> >::iterator it = arguments.begin(); it != arguments.end(); ++it)
      {
          if(ConstantExpr *e = dyn_cast<ConstantExpr>(*it))
          {
              cerr << "join ("<<i<<") ---\n";
              e->dump();
              cerr << "-------\n";
...
//:CLAP:
          ...
          std::vector<ref<Expr> > &arguments)
  {
      ...
      //Nuno: commented
      /*str = "wait_lock";
      executor.printSyncTrace(str,ss.str());*/
