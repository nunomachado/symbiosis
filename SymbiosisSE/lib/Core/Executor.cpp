...
const Module *Executor::setModule(llvm::Module *module,
    ...
    if(!tPath.empty())
    {
        std::cout << ">> Open trace file: "<< tPath << "\n";
        std::ifstream fin;
        fin.open(tPath.c_str());
        if(fin.good())
        {
	    //check whether trace corresponds to a failing or successfull execution
	    if(tPath.find(".fail")!=std::string::npos)
		failedExec = true;
	    else
		failedExec = false;
            ...
            while(!fin.eof())
            {
                ...
                string tid = line.substr(0,end-init);

		//check whether this is the thread with the assertion
		if(tid[0] == '['){
			tid = line.substr(1,tid.length()-1);
			assertThread = tid;
		}                
                ...
                int bbid = intValueOf(line.substr(init,end-init));
                
                threadbbiidsmap[tid].push_back(bbid);
                //cout << tid << ": " << bbid << "\n";
            }
        }
        else
        {
            cerr << ">> Error opening file "<< tPath <<"\n";
            cerr << " good()=" << fin.good();
            cerr << " eof()=" << fin.eof();
            cerr << " fail()=" << fin.fail();
            cerr << " bad()=" << fin.bad();
        ...
        for(std::map<std::string,std::vector<int> >::iterator oit = threadbbiidsmap.begin(); oit!=threadbbiidsmap.end(); ++oit)
        {
            cout  << "["<< oit->first <<"] ";
            std::vector<int> bbs = oit->second;
            for(std::vector<int>::iterator iit = bbs.begin(); iit!=bbs.end(); ++iit)
            {
                cout << *iit << " ";
            }
            cout << "\n";
    ...
    //Nuno: initialize the trace file with the readwrite constraints
...
Executor::fork(ExecutionState &current, ref<Expr> condition, bool isInternal) {
    ...
    if (isSeeding &&
        ...
        for (std::vector<SeedInfo>::iterator siit = it->second.begin(),
            ...
            //std::cerr << "Jeff: seeding solver 817\n";
    ...
    if (res==Solver::True) {
        ...
        //current.updateStateId(true);   //Nuno: update state Id
        ...
        //current.updateStateId(false); //Nuno: update state Id
        ...
        //Nuno: save the symbolic info collected so far
        tracePerThread[getThreadName()].erase(current.stateId); //delete the previous entry to free space
        
        tracePerThread[getThreadName()][falseState->stateId] = current.tmpTrace;
        falseState->tmpTrace = current.tmpTrace;
        
        tracePerThread[getThreadName()][trueState->stateId] = current.tmpTrace;
        ...
        if (MaxDepth && MaxDepth<=trueState->depth) {
            ...
            std::cerr << "["<<getThreadName()<<"] MAX-DEPTH EXCEEDED! -----\n";
            return StatePair(0, 0);
        }
...
void Executor::addConstraint(ExecutionState &state, ref<Expr> condition) {
  ...
  if (it != seedMap.end()) {
    ...
    for (std::vector<SeedInfo>::iterator siit = it->second.begin(), 
      ...
      //std::cerr << "Jeff: seeding solver 960\n";
  ...
  if (f && f->isDeclaration()) {
    ...
    case Intrinsic::not_intrinsic:
      // state may be destroyed by this call, cannot touch
    	//Nuno: calling external ...
            //std::cout << "callExternalFunction: "<<f->getName().data()<<"\n";  //***
...
//** Nuno: indicates whether all threads have already finished execution or not
bool Executor::allThreadsFinished()
{
    ...
    for(map<string,bool>::iterator iter = threadHasFinished.begin(); iter != threadHasFinished.end(); ++iter )
    {
        ...
        if(!tmp)
        {
            //std::cerr<<">>> allThreadsFinished["<<(*iter).first<<"] = "<< tmp <<"\n";
            ret = false;
            break;
        }
    }
...
{
    state.tmpTrace.append(line +":S-" + type + "-" + getThreadName() + "\n");
}
...
void Executor::runNewThread(ExecutionState &state,
    ...
    {
         //e->dump();
        //uint64_t ev = e->getConstantValue();
            ...
            iter!=globalAddresses.end();iter++)
        {
            if(iter->second==e)
            {
                ...
                if (const Function *f = dyn_cast<Function>(gv))
                {
                    ...
                    //set new trace file
                    //const std::string tracefile = "trace"+tname;
                    //interpreterHandler->setTraceFile(tracefile);
                    ...
                    //state.ptreeNode=0; //this should not be nullified, new a ptreeNode
                    state.ptreeNode = processTree->root;
                    
                    state.constraints.empty();
                    state.stack.clear();
                    state.pushFrame(0, kf);
                    //state.addressSpace //reuse addressSpace
                    //pathOS
                    //symPathOS
                    //shadowObjects
                    //incomingBBIndex
                    ...
                    //** Nuno: added to allow guiding the search in thread processes
                    searcher = constructUserSearcher(*this);
                    searcher->update(0, states, std::set<ExecutionState*>());
                    //((AvisoReplaySearcher*)searcher)->expandTraceFile();
                    //**
                    ...
                    while (!states.empty() && !haltExecution) //&& !((AvisoReplaySearcher*)searcher)->getTarget)
                    {
                        ...
                        //Nuno: terminate state when finding the last goal and save the information
                    ...
                    //** Nuno: added this to ensure that the constraints from this thread are written into the logfile
                    //terminateStateCLAPWithConstraints(*lastState);
...
//Nuno: enforce path only when we run with basic block traces
void Executor::newBasicBlockEntry(ExecutionState &state, std::vector< ref<Expr> > &arguments)
{
    int size = threadbbiidsmap[getThreadName()].size();
    if(!bbTrace.empty() && state.goalsReached < size)
    {
        //int iid = threadbbiidsmap[getThreadName()].front();
        //cerr << "\t[newBasicBlockEntry] T"<<getThreadName()<<": "<< state.goalsReached <<" GOALS\n";
        int iid = threadbbiidsmap[getThreadName()][state.goalsReached];
        ref<Expr> expr = arguments.front();
        
        int fail = expr.compare(ConstantExpr::create(iid, 32));
        
        //** if the function belongs to klee, then we don't have to check for the BBs
        string funcname = state.pc->inst->getParent()->getParent()->getNameStr();
        if(funcname.find("klee")!=std::string::npos)
        {
            cerr << "\t[newBasicBlockEntry] T"<<getThreadName()<<": function is "<< funcname<<" so allow to proceed regardless of the basic blocks\n";
            return;
        }
        ...
        if(fail)
        {
            ...
            std::cerr << "\t[newBasicBlockEntry] T"<<getThreadName()<<": BBtrace" <<iid<<" != BBprog"<< expr << "\n"; 
	    ...
	    if(state.goalsReached < size)
            {
                //threadbbiidsmap[getThreadName()].erase(threadbbiidsmap[getThreadName()].begin());
                state.goalsReached++;
            }
             std::cerr << "\t[newBasicBlockEntry] T"<<getThreadName()<<": BBtrace" << iid << " == BBprog"<< expr << " (reached "<< state.goalsReached <<" of "<< size <<" )\n"; //***
           
 	    if(assertThread == getThreadName()
		&& state.goalsReached == size)
	    {
		//end if this is the state corresponding to the assertion
		std::cerr << "["<< getThreadName() << "] Thread execution trace completed. Terminate execution.\n";
                terminateStateCLAPWithConstraints(state);
		exit(0);		
	    }
...
void Executor::transferToBasicBlock(BasicBlock *dst, BasicBlock *src,
	...
	/* //not using this at the moment (we are not removing BBs from the trace vector, but increasing a pointer instead)
    	if(threadbbiidsmap[getThreadName()].size()==0)
	{
		std::cerr << "["<< getThreadName() << "] All basic blocks are done. Terminate execution.\n";
		terminateStateCLAPWithConstraints(state);
		exit(0);
	}*/
...
void Executor::executeInstruction(ExecutionState &state, KInstruction *ki) {
  Instruction *i = ki->inst;
    ...
    std::cerr << "["<< getThreadName() << "] "<< state.stateId<< " executing " << extractFileBasename(info->file) << ":" << info->line << "\n";//" opcode - " << i->getOpcodeName() << "\n";//" (a: " << info->assemblyLine << ")\n";//
    //** Nuno: debug}
  ...
  case Instruction::Ret: {
	...
	//Nuno: check if all basic blocks were already executed before transfering
	if(state.goalsReached == threadbbiidsmap[getThreadName()].size())
            {
		std::cerr << "["<< getThreadName() << "] Thread execution trace completed. Terminate execution.\n";
                terminateStateCLAPWithConstraints(state);
		exit(0);
  ...
  case Instruction::Br: {
    ...
    if (bi->isUnconditional()) {
      ...
      //std::cerr << "Jeff: go branch unconditional\n";
    } else {
      ...
      std::cerr <<"["<<getThreadName()<<"] Branch Condition: "<<cond<<"\n";
  ...
  case Instruction::Switch: {
    ...
    if (ConstantExpr *CE = dyn_cast<ConstantExpr>(cond)) {
      ...
      for (std::map<BasicBlock*, ref<Expr> >::iterator it = 
        ...
        ExecutionState *es = *bit;
        if (es)
          transferToBasicBlock(it->first, bb, *es);//Jeff: need to decide which one to go here
        ++bit;
      }
    }
    break;
  ...
  case Instruction::Load: {
	...
	//Rewrite this function:
	  //1. simulate make_symbolic this variable first
	  //2. callExternalFunction - specialFunctionHandler->handle
	  //3. executeCall(state, ki, f, arguments);
	  // f - the klee_make_symbolic function
	  // arguments - the corresponding arguments
	  // klee_make_symbolic(&sd, sizeof(sd), "sd");

	  //YES, WE CAN DO IT!!!!!!

		//replaceReadWithSymbolic(state,base);

		ref<Expr> base = eval(ki, 0, state).value;

		  //1. MemoryObject *mo
		  //2. name std::string -> get from address??

		//speed up the address resolve process
		//from base to the closest memory object
		//getMemObjAddress(base)
		//
      ...
      for (ResolutionList::iterator it = rl.begin(), ie = rl.end(); it != ie; ++it)
      {
          ...
          if(isCorrectMemObj)
          {
              std::string name =  varaddrnamemap[mo->getBaseExpr()];
              if(name.length())
              {
                  ...
                  else
                  {
                      ...
                      //TODO: the address is symbolic!
                      //but what shall we do there?!
                      //std::cerr <<"READ TO SYMBOLIC ADDRESS "<<base<<" !\n";
                  ...
                  //std::cerr <<"===> [INST INFO] base: "<<base<<" line: "<<myinfo->line << " AssemblyLine: " << myinfo->assemblyLine << "\n"; //***
                  
                  mo->setName(name);
                  
                  //uniquename: objectname+threadname+index
                  ...
                  //Nuno: commented this to avoid print stuff to file when exploring unfeasible paths
                  /*traceFile << extractFileBasename(myinfo->file) << "@" << linestr.str() << ":"<< ss.str() << "\n";
                  if(!isa<ConstantExpr>(base))
                      traceFile <<"*"<<base<<"*\n";
                  traceFile.flush();*/
                  state.tmpTrace.append(extractFileBasename(myinfo->file) + "@" + linestr.str() + ":" + ss.str() + "\n");
                  if(!isa<ConstantExpr>(base))
                  {
                      stringstream ssbase;
                      ssbase <<"*"<<base<<"*\n";
        ...
        //std::cerr << "---- executeMemoryOperation LOAD\n";
  ...
  case Instruction::Store: {
	  ...
	  for (ResolutionList::iterator it = rl.begin(),
          ...
          if(isCorrectMemObj)
          {
              ...
              //std::cerr <<"STORE TO ADDRESS "<<name<<":"<<base<<"\n";
              //std::cerr <<tname<<" - Base: "<<base<<"  - Name: "<<name<<" - W "<<value<<"\n";
              
              if(value.isNull())
              {
                  std::cerr <<"["<<tname<<"] Store - base: "<<base<<"  name: "<<name<<"  value: null -> set value = 0\n";
                  value = ConstantExpr::create(0, 32);
              }
              
              if(name.length())
              {
                  std::cerr <<"["<<tname<<"] Store: "<<base<<"  - Name: "<<name<<" - value: "<<value<<"\n"; //Nuno: debug
                  ...
                  else
                  {
                      ...
                      //but what shall we do there?!
                      //just terminate, say we cannot handle this case??
                      //std::cerr <<"WRITE TO SYMBOLIC ADDRESS "<<base<<" !\n";
                      //add more constraints
                      //TODO
                  ...
                  std::stringstream ss;
                  ss << "W-" << name<<"_"<<basevalue<<"-"<<getThreadName();
      ...
      //std::cerr << "---- executeMemoryOperation STORE\n";
...
void Executor::run(ExecutionState &initialState) {
  ...
  if (usingSeeds) {
    ...
    //** Nuno: changed CLAP to use a state searcher;
    while (!states.empty() && !haltExecution)
    {
        ExecutionState &state = searcher->selectState();
        if(!bbTrace.empty())
        {
            //** Nuno: when using a basic block trace we don't have to search for the next state
            KInstruction *ki = state.pc;
            stepInstruction(state);
        ...
        else //** Nuno: when using Aviso trace
        {
            //Nuno: terminates the executor if searcher asks to do so.
            if (searcher->empty()) { //&& allThreadsFinished()){
...
void Executor::terminateStateCLAPWithConstraints(ExecutionState &state) {
    ...
    if(!state.tmpTrace.empty())
        tracePerThread[getThreadName()][state.stateId] = state.tmpTrace;
    
    //std::ostream &traceFile = interpreterHandler->getTraceStream();
    //traceFile << "\n\n[T"<< getThreadName() << "] Last State = "<< state.stateId<<" ("<< state.goalsReached<<" goals)-------------------\n";
    
    //build tracefile with the symbolic information gathered
    /*for(map<string, string>::iterator it = tracePerThread[getThreadName()].begin(); it != tracePerThread[getThreadName()].end(); ++it)
    {
        traceFile << "\n###State " << it->first << "\n";
        traceFile << it->second;
    }//*/
    
    //** use the global traceFile
    /*traceFile << tracePerThread[getThreadName()][state.stateId];  //log only the final state
    traceFile.flush();*/
    
    //** NEW: use a single tracefile per path
    string tracefilename = "T"+getThreadName()+"_"+state.stateId;
    ...
    //stateTraceFile << "\n\n[T"<< getThreadName() << "] Last State = "<< state.stateId<<" ("<< state.goalsReached<<" goals)-------------------\n";
    stateTraceFile << tracePerThread[getThreadName()][state.stateId];
    stateTraceFile.flush();
    
    //Nuno: insert path constraints
    ...
    for(std::vector< ref<Expr> >::const_iterator cb = state.constraints.begin(), ce=state.constraints.end();cb!=ce;cb++)
    {
        if(Expr *e = dyn_cast<Expr>(*cb))
        {
            //e->dump();
            /*traceFile<<"T"<<getThreadName()<<":";
            e->print(traceFile);
            traceFile<<"\n";*/
            stateTraceFile<<"T"<<getThreadName()<<":";
            e->print(stateTraceFile);
            stateTraceFile<<"\n";
        }
    }
    std::cerr << "[" << getThreadName() << "_" << state.stateId <<"] terminateStateCLAPWithConstraints\n";
    ...
    stateTerminate = true;
    std::cerr<<"Thread "<<getThreadName()<<" terminated. Flush Constraint Trace\n"; //***
    endTime = clock();
    std::cerr << "Elapsed Time: " << (double)(endTime-beginTime)/(double) CLOCKS_PER_SEC << "s\n";
    std::cerr<< "------------------\n";
    //exit(0);
}
...
void Executor::terminateStateEarly(ExecutionState &state, 
  ...
  if (!OnlyOutputStatesCoveringNew || state.coveredNew ||
    ...
    std::cerr << "[" << getThreadName() << "] terminateStateEarly ----- \n";
    
    //Nuno: modified this for ending bugredux with CLAP constraints
    //UPDATE: originally terminateState was commented, but I changed it to avoid storing in the trace states that not reach all goals
...
void Executor::terminateStateOnExit(ExecutionState &state) {
  if (!OnlyOutputStatesCoveringNew || state.coveredNew || 
    ...
    std::cerr << "[" << getThreadName() << "] terminateStateOnExit ----- \n";
 
    
  //modified by Jeff -> Nuno: I commented terminateStateCLAPWithConstraints and left the other
   //terminateState(state);
  terminateStateCLAPWithConstraints(state);
}
...
void Executor::terminateStateOnError(ExecutionState &state,
  ...
  const InstructionInfo &ii = *state.prevPC->info;
    ...
    std::cerr << "[" << getThreadName() << "] terminateStateOnError ----- state "<< state.stateId <<"@"<< ii.line <<"\n";
  ...
  //Modified by Jeff
    //TODO: if error occurs, don't terminate the state -> Nuno: I uncommented this (was commented initially)
  //terminateState(state);
    
    //Nuno: terminate state with constraints if this corresponds to the last bb in the trace
    if(threadbbiidsmap[getThreadName()].size() == 0)
        terminateStateCLAPWithConstraints(state);
}
...
void Executor::callExternalFunction(ExecutionState &state,
  ...
  //std::cerr << "executeCall 3486 -> function "<< function->getName().str() << "\n"; //** Nuno: debug
...
void Executor::executeMemoryOperation(ExecutionState &state,
	...
	//FIX: BY JEFF HUANG
	//If write or read to a symbolic address, just return or exit?
//	if(!isa<ConstantExpr>(address))
//	{
//		std::cerr <<"READ/WRITE TO SYMBOLIC ADDRESS! thread-address-value: "<<tname<<"-"<<address<<"-"<<value<<"\n";
//		exit(0);//terminate, say we cannot handle this case?
//		return;
//	}
   
    
    //** Nuno: there are cases where value can be null, I'm trying to address this by checking that and writing value 0 (is this a good way?)
    //** added if(value == null) else blocks
    Expr::Width type;
    if(isWrite && value.isNull())
    {
        std::cerr << "\t["<<getThreadName()<<"] executeMemoryOperation: isWrite and value is null -> set value = 0\n";
        value = ConstantExpr::create(0, 32); //create value zero
        type = value->getWidth();
    }
    else
    {
        //** Nuno: this was the original code
  ...
  if (!replayOut) {
    ...
    // or if that fails try adding a unique identifier.
	  ...
	  if(id>0)
	  {
		  std::stringstream ss;
		  ss << id;
		  uniqueName = uname+"-" +getThreadName()+"-"+ss.str();

		  state.arrayNames[uname] = ++id;
	  }
	  else
	  {
		  uniqueName = uname+"-" +getThreadName()+"-0";
		  state.arrayNames[uname]=1;

		  varaddrnamemap[mo->getBaseExpr()]=name;

		  //this would memory inefficient, but will work

//		  for(uint64_t k=0;k<mo->size;k++)
//		  {
//			  varaddrnamemap[Expr::createPointer(mo->address+k)]=name;
//		  }


		  //std::cerr << "Base: "<<mo->getBaseExpr()<<"  - Size: "<<mo->getSizeExpr()<<"  - Name: "<<name<<"\n";
	  }


//    while (!state.arrayNames.insert(uniqueName).second) {
//      uniqueName = name +"-" +tname+"-" + llvm::utostr(++id);
//    }
